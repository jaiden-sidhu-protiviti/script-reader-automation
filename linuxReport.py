...
def load_key_vars_linux(data):
    ...
    def calc_complexity():
        try:
            ...
            complexity = 0
            if "-1" in str(complexity_vars.get("dcredit", "")): complexity += 1
            if "-1" in str(complexity_vars.get("lcredit", "")): complexity += 1
            if "-1" in str(complexity_vars.get("ucredit", "")): complexity += 1
            if "-1" in str(complexity_vars.get("ocredit", "")): complexity += 1
            return complexity
        except Exception:
            return "Not set"
    ...
    return {
        "Key Variables": {
            ...
            "Lockout Duration": safe(lambda: data.get("login_attempts").get("unlock_time")),
...
def evaluate_from_json(data, all_files, cheat_sheet):
    ...
    if not found_insecure:
        ...
        checks = {
            ...
            "<b>Host-based authentication disabled</b>": ssh.get("HostbasedAuthentication") == "no",
            "<b>Empty passwords disabled</b>": ssh.get("PermitEmptyPasswords") == "no",
            "<b>MaxAuthTries ≤ 4</b>": str(ssh.get("MaxAuthTries")).isdigit() and int(ssh.get("MaxAuthTries")) <= 4
        }
        ...
        if ssh.get("ciphers"):
            findings_225b.append(f"<b>Strong ciphers configured</b>: {', '.join(ssh.get('ciphers'))}")

        if ssh.get("MACs"):
            findings_225b.append(f"<b>MAC algorithms configured</b>: {', '.join(ssh.get('MACs'))}")

        if ssh.get("KexAlgorithms"):
            findings_225b.append(f"<b>KEX algorithms configured</b>: {', '.join(ssh.get('KexAlgorithms'))}")
    ...
    generic_users = [
        u["username"] for u in passwd_list
    ...
    add(
        ...
        "passed" if summary.get("Telnet") == "FALSE" and ssh.get("Protocol") == "2" else "failed",
        [
    ...
    findings_227c.append(f"SSH Protocol = {ssh.get('Protocol')}: {'PASS' if protocol_ok else 'FAIL'}")

    # Authentication enforced
    auth_ok = (
        ssh.get("PermitEmptyPasswords") == "no"
        and summary.get("NullOK") != "TRUE"
    )

    findings_227c.append(f"PasswordAuthentication = {ssh.get('PasswordAuthentication')}: {'PASS' if ssh.get("PermitEmptyPasswords") == "no" else 'FAIL'}")
    findings_227c.append(f"NullOK = {summary.get('NullOK')}: {'PASS' if summary.get('NullOK') != 'TRUE' else 'FAIL'}")
    findings_227c.append(f"CertAuth = {summary.get('CertAuth')}: {'PASS' if summary.get('CertAuth') == 'TRUE' else 'FAIL'}")
    ...
    generic_markers = [
        "shared", "generic", "functional", "svc", "service", "admin", "test", "temp", "bootstrap"
    ]
    ...
    for user in enabled_users:
        ...
        if username == "root":
            findings_821b.append("Root is enabled for interactive access; review whether it is used for routine administration.")
            continue

        if any(marker in username for marker in generic_markers) or any(marker in comment for marker in generic_markers):
            suspect_accounts.append(user.get("username"))

    if enabled_users == "no file found" or not enabled_users:
        status_821b = "manual"
        findings_821b.append("8.2_enabledusers.txt was not found. Unable to assess enabled interactive accounts.")
    else:
        findings_821b.append(f"Enabled interactive accounts observed: {', '.join(u['username'] for u in enabled_users)}")
    ...
    if user_changes == "no file found":
        status_824 = "manual"
        findings_824.append("8.2.4_user_changes.txt was not found. Unable to evaluate account modification activity.")

    elif isinstance(user_changes, list) and len(user_changes) == 0:
        status_824 = "review"
        findings_824.append("No recent useradd/usermod/userdel changes were observed in 8.2.4_user_changes.txt.")
        findings_824.append("Still requires review of change tickets / approvals to confirm the activity has been managed.")

    else:
        status_824 = "review"
        findings_824.append(f"Observed {len(user_changes)} recent user/account change event(s).")
        findings_824.append("Review supporting approval/ticket evidence to confirm changes were authorized and implemented appropriately.")
    ...
    STRONG_HASHES = {"6", "y", "gy", "7"}   # $6$=SHA-512, $y$=yescrypt, $gy$=gost-yescrypt, $7$=scrypt
    ...
    if enc:
        ...
        findings_832b.append("shadow.txt is present but password hash values are redacted.")
        findings_832b.append(
    ...
    cipher_str   = ", ".join(ciphers_832c) if isinstance(ciphers_832c, list) and ciphers_832c else "None"
    ...
    summary_minlen_ok = (
        pw_min not in (None, "", "NA") and
        str(pw_min).isdigit() and int(pw_min) >= 12
    )
    ...
    credits_ok = all(
        credit_negative(v) for v in [pq_dcredit, pq_lcredit, pq_ucredit, pq_ocredit]
    ...
    if not pq_minclass and not any(v for v in [pq_dcredit, pq_lcredit, pq_ucredit, pq_ocredit]):
        findings_836.append(
    ...
    if pwquality == "no file found":
        ...
        findings_836.append("Password policy sourced from pwquality configuration file.")
    ...
    if pw_ages == "no file found":
        status_839 = "manual"
        findings_839.append("8.3.9_pw-ages.csv was not found. Cannot evaluate password rotation policy.")

    elif isinstance(pw_ages, list) and len(pw_ages) == 0:
        status_839 = "review"
        findings_839.append("8.3.9_pw-ages.csv was present but no password age records were parsed.")
        ...
        if noncompliant:
            status_839 = "review"
            findings_839.append("The following accounts do not meet the 90-day password rotation requirement:")
            ...
            status_839 = "passed"
            findings_839.append("All observed accounts have a maximum password age of 90 days or less.")
    ...
    has_auth     = any(t in ("auth", "authpriv", "debug", "info", "iinfo") for t in log_level_tags)
    ...
    add(
        ...
        ) if status_10213 == "passed" else ""
    )

    # -------------------------
    # [10.2.1.4] - INF-Cloud-LX-5770
    # -------------------------
    status_10214 = "passed" if (log_is_active and has_auth and attempts_configured) else "review"
