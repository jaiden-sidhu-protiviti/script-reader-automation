...
def run_pipeline(sample_folders, progress_callback=None):
    ...
    for i, folder in enumerate(sample_folders):
        ...
        if os_type == "windows":
            ...
            hostname      = (
                data.get("summary", {}).get("Host") or
                data.get("uname",   {}).get("host") or
                "Unknown Host"
            )
            slug          = slugify(hostname)
    ...
    for idx, meta in enumerate(hosts_meta):
        ...
        nav_links = {
            ...
            "prev": os.path.basename(hosts_meta[idx - 1]["report_path"]) if idx > 0 else None,
            "next": os.path.basename(hosts_meta[idx + 1]["report_path"]) if idx < len(hosts_meta) - 1 else None
        }
    ...
    windowsReport.render_homepage(site_reports, output_path=index_path, report_session=shared_report_session)
    ...
    def _add_folder(self):
        if len(self.folders) >= MAX_FOLDERS:
            messagebox.showwarning(
                "Limit Reached",
                f"Maximum of {MAX_FOLDERS} folders allowed."
            )
            return
