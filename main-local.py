...
def main():
    # Define a list of folders you want to process here
    # These folders must be in the same directory as this script or provide the correct relative/absolute path
    ...
    for idx, meta in enumerate(hosts_meta):
        nav_links = {
            "home": "../../index.html",
            "prev": hosts_meta[idx - 1]["report_path"].split("/")[-1] if idx > 0 else None,
            "next": hosts_meta[idx + 1]["report_path"].split("/")[-1] if idx < len(hosts_meta) - 1 else None
        }
